...
module.exports = {
    getPrices: async function (coins) {
        ...
        let prices = {};
        (async () => {
            const priceData = await connection.getLatestPriceFeeds(address_to_process);

            priceData.map(data => {
                const priceObject = data.price
                let decimalPrice = parseFloat(priceObject.price) * Math.pow(10, priceObject.expo);
                ...
                if (timeDifference <= MAX_TIME_DIFFERENCE) {
                    const coin =  Object.keys(coins)
                        .filter((address) => coins[address].pyth === `0x${data.id}`);
                    if (coin.length) {
                        prices[coin[0]] = decimalPrice;
                    }
                }
                else {
                    console.error(`Invalid publishTime. It is less than ${MAX_TIME_DIFFERENCE} seconds from now.`);
                }
            });
